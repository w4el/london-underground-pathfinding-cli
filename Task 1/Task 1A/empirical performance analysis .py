...
import os
from Task_1A import ShortestPathFinder, StationLineFinder  # Assuming Task_1A contains these classes.
from Graph import Graph_journey_duration  # Assuming Graph.py contains this class.

# Define a class to record performance of the shortest path finding algorithm.
class PerformanceRecorder:
    # Initialize with a directory to save the file and a default filename.
    def __init__(self, directory, filename='performance_analysis.xlsx'):
        ...
        self.filename = filename  # Filename for the saved Excel file.

    # Method to add a new performance record.
    def add_record(self, num_lines, num_stations_per_line, time_taken):
        # Append a dictionary with performance data to the records list.
        self.records.append({
    ...
    # Method to save the records to an Excel file.
    def save_to_excel(self):
        df = pd.DataFrame(self.records)  # Convert the records list to a DataFrame.
        summary = df.describe()  # Generate summary statistics.
        filepath = os.path.join(self.directory, self.filename)  # Create the full file path.
        
        # Write data to Excel with two sheets: raw data and summary.
        with pd.ExcelWriter(filepath, engine='xlsxwriter') as writer:
            df.to_excel(writer, sheet_name='Raw Data', index=False)
            summary.to_excel(writer, sheet_name='Summary', index=True)

            # Create a line chart in the Excel file to visualize the data.
    ...
    def generate_synthetic_data(num_lines, num_stations_per_line, num_interchange_stations):
        # Create synthetic data for the tube lines and stations.
        tube_lines = [f'Line_{i}' for i in range(1, num_lines + 1)]
        data = []
        interchange_stations_dict = {}

        # Generate stations and line data.
        ...
        # Generate interchange connections.
...
# Function to analyze the performance of the shortest path algorithm.
